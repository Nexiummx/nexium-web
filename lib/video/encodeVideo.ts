 ...
 * en un MP4 H.264 optimizado para reproducción y plataformas sociales.
 *
...
export async function encodeVideo(options: EncodeVideoOptions): Promise<void> {
  ...
  const vf = `scale=${safeW}:${safeH},pad=ceil(iw/2)*2:ceil(ih/2)*2`;
  ...
  return new Promise((resolve, reject) => {
    ffmpeg()
      ...
      .outputOptions([
        ...
        `-vf ${vf}`,
        // El header del MP4 al inicio para mejor streaming
        "-movflags +faststart",
        `-r ${fps}`,
      ])
      .output(outputPath)
